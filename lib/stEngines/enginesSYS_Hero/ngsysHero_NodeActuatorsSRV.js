...
class NGSYS_Hero_Node_ActuatorsSRV extends ActuatorsServices {
	...
	_mapActuatorControlEvents(actuator) {
		...
		actuator.eventEmitter.on(actuator.CONSTANST.Events.ActuatorOptionsUpdated, service._event_ActuatorOptionsUpdated);
	...
	_mapControlMessages(socket) {
		...
		socket.on(service.CONSTANTS.Messages.getActuatorsList, service._msg_getActuatorsList);
		socket.on(service.CONSTANTS.Messages.getActuatorOptions, service._msg_getActuatorOptions);
		socket.on(service.CONSTANTS.Messages.setActuatorOptions, service._msg_setActuatorOptions);
		socket.on(service.CONSTANTS.Messages.StartActuator, service._msg_StartActuator);
		socket.on(service.CONSTANTS.Messages.StopActuator, service._msg_StopActuator);
		socket.on(service.CONSTANTS.Messages.TurnOffActuators, service._msg_TurnOffActuators);
	...
	_msg_getActuatorsList(msg) {
		...
		let response = {};
		response.numActuators = amng.actuatorList.length;
		response.actuators = [];
	  
		amng.actuatorList.forEach(function(act_, _i) {
