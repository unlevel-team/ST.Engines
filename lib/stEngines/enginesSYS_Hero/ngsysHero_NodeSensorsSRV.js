...
//		socket.removeListener(service.CONSTANTS.Messages.TurnOffSensors, service._msg_TurnOffSensors);
	...
	_msg_getSensorOptions(msg, service) {
		...
		try {
			...
			/*
			sensor.getOptions().then(function(_data) {
				
				response.options = _data;
				...
				throw 'Error in sensor options. ' + _error;
			});
			*/
	...
	_msg_setSensorOptions(msg, service) {
		...
		try {
			...
			/*
			sensor.setOptions(options).then(function(_data){
...
//		  console.log(' <~> ' + msg);	// TODO REMOVE DEBUG LOG
		...
		try {
			...
			if (_sensorSearch.STsensor !== null) {
				...
				/*
				_sensorSearch.STsensor.sensorEngine.startEngine().then(function(_data) {
					response.result = "OK";
				}, function(_error) {
					response._STsensor = _sensorSearch.STsensor;
					throw 'Error starting engine. ' + _error;
				});
				*/
...
//		console.log(' <~> ' + msg);	// TODO REMOVE DEBUG LOG
		...
		try {
		  ...
		  if (_sensorSearch.STsensor !== null) {
			  ...
			  /*
			  _sensorSearch.STsensor.sensorEngine.stopEngine().then(function(_data) {
				  response.result = "OK";
			  }, function(_error) {
					throw 'Error stopping engine. ' + _error;
			  });
			  */
